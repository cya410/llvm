...
void ARMConstantIslands::InsertJump()
{
	for (MachineFunction::iterator MBBI = MF->begin(); MBBI != MF->end(); MBBI++)
	{
		...
		if (MBBI->canFallThrough())
		{
			MachineBasicBlock::iterator InsAt = MBBI->end();
			InsAt--;
			DebugLoc dl   = InsAt->getDebugLoc();
			TII->InsertBranch(*MBBI, NextBB, nullptr, SmallVector<MachineOperand, 0>(), dl);
		}
	}
}
...
/********************************************************************************/		
void ARMConstantIslands::doInitialPlacement(std::vector<MachineInstr*> &CPEMIs) {
